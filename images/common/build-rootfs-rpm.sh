...
# ca-certificates ships its TLS bundle as %ghost files generated by the
# %post scriptlet (update-ca-trust). Under --installroot that scriptlet
# can run before the target's shell is laid down and silently leave a
# dangling /etc/pki/tls/certs/ca-bundle.crt symlink (Fedora). Regenerate
# inside the chroot when the bundle is missing. No-op on EL9/EL10.
if [ ! -e "${ROOTFS}/etc/pki/tls/certs/ca-bundle.crt" ]; then
